...
async function loadFriends() {
  ...
  try {
    ...
    showToast('فشل تحميل قائمة الأصدقاء', 'error');
  }
}
...
async function loadMessages() {
  ...
  try {
    const res = await fetch(`/api/messages/${currentFriendId}`, { 
      headers: { 'Authorization': `Bearer ${token}` } 
    });
    ...
    if (msg.file_type && msg.file_type.startsWith('image/')) {
      ...
      contentHtml += `
        <a href="${msg.file_url}" target="_blank" class="file-attachment">
          ...
          <span>${msg.content || 'ملف مرفق'}</span>
        </a>
      `;
    }
  }
...
socket.on('receive_message', (msg) => {
  if (msg.sender_id === currentFriendId) {
    ...
    // يمكن إضافة إشعار هنا لصديق آخر
...
async function checkCatchmail() {
  ...
  try {
    const res = await fetch('/api/catchmail', { 
      headers: { 'Authorization': `Bearer ${token}` } 
    });
      ...
      data.forEach(msg => {
        container.innerHTML += `
          <div style="background:var(--bg-dark); padding:15px; margin:8px 0; border-radius:8px; border-right:3px solid var(--primary-color);">
            <div style="font-weight:600; margin-bottom:5px; color:var(--primary-light);">
              ${msg.from || 'مجهول'}
            </div>
            ...
            <div style="font-size:0.8rem; color:var(--text-muted);">
              ${new Date(msg.date || Date.now()).toLocaleString('ar-EG')}
            </div>
          </div>
        `;
      });
...
async function handleAddFriend(e) {
  ...
  if (!identifier) {
    showToast('يرجى إدخال اسم المستخدم أو البريد', 'warning');
    return;
  }
...
async function startVoiceCall() {
  if (!currentFriendId) {
    showToast('الرجاء اختيار صديق أولاً', 'warning');
    return;
  }
